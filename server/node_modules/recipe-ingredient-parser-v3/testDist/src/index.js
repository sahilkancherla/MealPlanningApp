...
function parse(recipeString, language) {
    var _a;
    var ingredientLine = recipeString.trim().replace(/^(-)/, "").toLowerCase(); // removes leading and trailing whitespace
    ...
    var regex_originalunit = RegExp('^' + originalUnit + '|' + originalUnit + '$');
    var regex_unit = RegExp('^' + unit + '|' + unit + '$');
    var ingredient = !!originalUnit ? restOfIngredient.replace(regex_originalunit, '').trim() : restOfIngredient.replace(regex_unit, '').trim();
